...
**First check who is asking.** If `whoami` returns `actingAsTenant: true`, the caller is a customer working inside a partner's product against their own workspace: it is **Path A, not asked and not discussed** — no manifest talk, no "your customers" framing. The rest of this section applies to a partner's own staff (`actingAsTenant: false`).
